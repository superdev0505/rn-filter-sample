...
export const getFilteredUsers = (
  ...
  new Promise(resolve => {
    const regexpsgender = gender
      .split(' ')
      .map(word => new RegExp(String(word), 'g'))

    const genderFilteredData = users.filter(item =>
      regexpsgender.every(regex =>
        ['gender'].some(key => regex.test(item[String(key)]))))

    const regexpsskill = skill
      .split(' ')
      .map(word => new RegExp(String(word), 'g'))

    const filteredData = genderFilteredData.filter(item =>
      regexpsskill.every(regex =>
        ['skill'].some(key => regex.test(item[String(key)]))))
